...
addpath 'C:\Users\imraj\Documents\UCL\COMP0121\Coursework_4\Bloch_Solver\Functions'
...
x1 = linspace(-L/2 + dxy/2 - d, +L/2 - dxy/2 - d,Numxy);
y1 = linspace(-L/2 + dxy/2 - d, +L/2 - dxy/2 - d,Numxy);
...
for i=2:NumGy
    blocks.N = [blocks.N blocks.N(1) blocks.N(3) blocks.N(4)];
    % Time over which the block occurs
    blocks.t = [blocks.t 0 Ts/2 Ts];
    
    % The gradient of the field dB/dx for each block
    blocks.Gx = [blocks.Gx 0 Gradient -Gradient];
    
    % The gradient of the field dB/dz for each block
    blocks.Gy = [blocks.Gy 0 Gradient_y(i) 0];
    
    % The relaxation time only when spins are relaxation
    blocks.relax = [blocks.relax 0 Ts/2 Ts];
    
    % Reset k-space
    blocks.resetk = [blocks.resetk 1 0 0];
    
    % Where the signal is collected
    blocks.signal = [blocks.signal 0 0 1];
end
...
Mxrec = zeros(NumGx*NumGy,1);
Myrec = zeros(NumGx*NumGy,1);
Mzrec = zeros(NumGx*NumGy,1);


% Subset definition
Sub_num = 2;
Sub_Set = [ min(x1) max(x1)  min(x1) max(x1)  ];
...
for i=1:length(iso)
    ...
    iso(i) = free_relaxation(iso(i), blocks.N(3), blocks.t(3), ...
        blocks.Gx(3), blocks.Gy(3), blocks.relax(1:3), gamma, T1, T2, R_FoR,0);
    
    % Free relaxation with dephasing readout
    iso(i) = free_relaxation(iso(i), blocks.N(4), blocks.t(4), ...
        blocks.Gx(4), blocks.Gy(4), blocks.relax(1:4), gamma, T1, T2, R_FoR,1);
    
    
    for ii=2:NumGy
        index = ii-1+(ii-2)*2;
        % Initialise spin
        iso(i) = initial_dir(iso(i),'y',blocks.N(4+index));
        
        % Free relaxation with phase encoding gradient
        iso(i) = free_relaxation(iso(i), blocks.N(5 + index), blocks.t(5 + index), ...
            blocks.Gx(5+index), blocks.Gy(5+index), blocks.relax(1:3), gamma, T1, T2, R_FoR,0);
        
        % Free relaxation with dephasing readout
        iso(i) = free_relaxation(iso(i), blocks.N(6 + index), blocks.t(6 + index), ...
            blocks.Gx(6+index), blocks.Gy(6+index), blocks.relax(1:3), gamma, T1, T2, R_FoR,1);
    end
    
    % Calculate the index's for the map array dependent on spatial location
    ix = round((iso(i).x + L/2 - dxy/2 + d)/(x1(2)-x1(1)) + 1);
    iy = round((iso(i).y + L/2 - dxy/2 + d)/(y1(2)-y1(1)) + 1);
...
%     Mxrec = Mxrec + iso(i).Mxrec;
%     Myrec = Myrec + iso(i).Myrec;
%     Mzrec = Mzrec + iso(i).Mzrec;
...
My = My/(length(iso));
Mz = Mz/(length(iso));
Mxrec = Mxrec/(length(iso));
Myrec = Myrec/(length(iso));
Mzrec = Mzrec/(length(iso));
...
%% Extract the signal... idk how to do yet

% NEED
% - All the k-space signals and the Magnetisation then...

imagesc(linspace(-L/2 + dxy/2 - d,L/2 - dxy/2 + d,100)*1000,linspace(-L/2 + dxy/2 - d,L/2 - dxy/2 + d,100)*1000,map(:,:,164),[0 360])
...
for i=1:length(Time)
    ...
    if Time(i)>=0 && Time(i)<sum(blocks.t(1:2))
        imagesc(linspace(-L/2 + dxy/2 - d,L/2 - dxy/2 + d,100)*1000,linspace(-L/2 + dxy/2 - d,L/2 - dxy/2 + d,100)*1000,ones(length(map(:,:,1)))*-1,[0 360])
    elseif Time(i)>=sum(blocks.t(1:2))
        imagesc(linspace(-L/2 + dxy/2 - d,L/2 - dxy/2 + d,100)*1000,linspace(-L/2 + dxy/2 - d,L/2 - dxy/2 + d,100)*1000,map(:,:,i),[0 360])
    end
    hold on
    set(gca,'YDir','normal');
    grid on
    ...
    % Set the axes limits
    if Time(i)>=0 && Time(i)<sum(blocks.t(1:4))
        xlim([0 sum(blocks.t(1:4))]*1000)
    elseif Time(i)>=sum(blocks.t(1:4))
        xlim([Time(i)-sum(blocks.t(1:4)) Time(i)]*1000)
    end
    ylim([-1 1]*1.2);
    % Set the axes labels
