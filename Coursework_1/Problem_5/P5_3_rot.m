...
%% Sensitivity of Cauchy to omega_0
% Author: Imraj Singh
...
for Di = 1:length(delta)
    ...
    for i=1:length(time)
        Mperp(i,1,Di) = exp(-time(i)/T2)*sqrt(sum(Msoln(i,1,:))^2 + sum(Msoln(i,2,:))^2)/Num;
        Mperp(i,2,Di) = exp(-time(i)/T2)*sum(Msoln(i,1,:))/Num;
        Mperp(i,3,Di) = exp(-time(i)/T2)*sum(Msoln(i,2,:))/Num;
    end
end

h = figure;
for i=1:length(delta)
    plot(time(:),Mperp(:,2,i),'linewidth',2,'LineStyle','-','Color','r')
    hold on
    plot(time(:),Mperp(:,3,i),'linewidth',2,'LineStyle','-','Color','b')
    ...
    ylabel("Normalised average $M_\perp$", "interpreter", "latex", "fontsize", 15)
    legend("$ \frac{1}{N} \sum M_{x'}$","$\frac{1}{N} \sum M_{y'}$","$T_2$ envelope", "interpreter", "latex", "fontsize", 10)
    
    frame(i) = getframe(h);
end
video = VideoWriter(['5_3_omega_comp', '.mp4'], 'MPEG-4');
frameRate = .25;
