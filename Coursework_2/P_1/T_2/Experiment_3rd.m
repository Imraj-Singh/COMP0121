...
for i=1:length(Time)
    ...
    if Time(i)>=0 && Time(i)<sum(blocks.t(1:2))
        imagesc(linspace(-L/2,L/2,Numx*2)*1000,linspace(-L/2,L/2,Numx*2-1)*1000,ones(length(map(:,1,1)))*-1,[-1 360])
...
%     end
    ...
    % Set the axes limits
     xlim([-.6 .6]);
     ylim([(min(ky)/1000-0.2) (max(ky)/1000+0.2)]);
    % Set the axes labels
    ...
    % Set the axes limits
    xlim([0 max(Time)*1000]);
    ylim([(min(kx)/1000-0.2) (max(kx)/1000+0.2)])
    % Set the axes labels
    xlabel("Time (ms)", "interpreter", "latex", "fontsize", 10)
