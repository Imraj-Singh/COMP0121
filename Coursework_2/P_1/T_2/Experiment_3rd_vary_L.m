...
% Subset definition
Sub_dx = 1/1000;
Sub_set = linspace(-L/2 + (Sub_dx)/2,L/2 - (Sub_dx)/2,L/Sub_dx);
...
for i=1:length(iso)
    ...
    for k=1:sum(blocks.N)
        map(:,ix,k) = ceil(mod(atan2(iso(i).My(k), iso(i).Mx(k)),2*pi)/(pi)*180);
    end
...
for i=1:length(Time)
    ...
    sgtitle(['Recording signal, time (after dephase): ', num2str((Time(i)-sum(blocks.t(1:3)))*1000), ' ms'], "interpreter", "latex", "fontsize", 15)
    end
    ...
    if Time(i)>=0 && Time(i)<sum(blocks.t(1:2))
        imagesc(linspace(-L/2,L/2,Numx)*1000,linspace(-L/2,L/2,Numx)*1000,ones(length(map(:,1,1)))*-1,[-1 360])
    elseif Time(i)>=sum(blocks.t(1:2))
        imagesc(linspace(-L/2,L/2,Numx)*1000,linspace(-L/2,L/2,Numx)*1000,map(:,:,i),[-1 360])
    end
    hold on
